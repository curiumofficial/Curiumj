...
import org.curiumj.core.listeners.BlockChainListener;
import org.curiumj.store.FlatDB;
import org.curiumj.store.HashStore;
import org.curiumj.store.MasternodeDB;
 ...
 * curiumj for a specific network. You can get an instance of this class through calling {@link #get()}.</p>
 *
...
public class Context {
    ...
    public Context(NetworkParameters params) {
        log.info("Creating curiumj {} context.", VersionMessage.BITCOINJ_VERSION);
     ...
     * independently use curiumj (or possibly alt coin forks of it) can operate correctly.
     *
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a curiumj context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using curiumj!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing curiumj via a thread that has not had any context set on it.");
    ...
    /**
     * Require that new threads use {@link #propagate(Context)} or {@link org.curiumj.utils.ContextPropagatingThreadFactory},
     ...
     * a {@link org.curiumj.utils.ContextPropagatingThreadFactory}.
     */
    ...
    /**
     * Returns the {@link org.curiumj.core.NetworkParameters} specified when this context was (auto) created. The
